...
# Уже разобрано в отдельные поля либо технический шум — в карточке лида не нужно.
_SKIP_ANSWER_KEYS = {"телефон", "имя", "id", "сайт", "устройство"}
...
def _parse_lead_channel(text: str, pairs: list[tuple[str, str]]) -> dict:
    """Канальный формат: «Сайт/Источник/Лендинг/[Имя]/Телефон/…/ID».

    🛑 Поля плавают: у формы обратного звонка есть только телефон, у квиза
    добавляются «Имя» и ответы (регион, сумма долга, кредиторы, имущество…).
    Поэтому берём ВСЕ пары «Ключ: значение», а не фиксированный список —
    новый вопрос в квизе доедет в карточку лида сам, без правки кода."""
