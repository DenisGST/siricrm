...
_SUBTYPE_TEXT_TEMPLATES = {
    "PropertySale": (
        ...
        "{срок процедуры} месяцев. Финансовым управляющим {утверждён ФУ} "
    ...
    "DebtRestructuring": (
        "{дата решения}г. {арбитражный суд} вынес определение по делу № {номер дела}, "
        "должник {ФИО должника} ({дата рождения} г/р, уроженец: {место рождения}, "
        "адрес: {адрес регистрации}, ИНН {ИНН}, СНИЛС {СНИЛС}) признан несостоятельным "
        "(банкротом). В отношении должника введена процедура реструктуризации долгов "
        "гражданина. Финансовым управляющим {утверждён ФУ} {ФИО Финансовый управляющий} "
        ...
        "{дата следующего заседания}, в помещении {арбитражного суда} "
        "по адресу: {адрес суда}."
    ),
    # 🛑 ФИО не склоняем (из CRM — именительный): фразы построены под именительный
    # через тире (конвенция apps.kommersant). Вознаграждение 25 000 ₽ — фикс. по
    # ст. 20.6 ФЗ-127 для реализации; при иной сумме АУ правит текст.
    "PropertySaleComplete": (
        ...
        "введена процедура реализации имущества гражданина сроком на {срок процедуры} "
        "месяцев. Финансовым управляющим {утверждён ФУ} {ФИО Финансовый управляющий} "
        "(ИНН {ИНН АУ}, СНИЛС {СНИЛС АУ}, адрес для корреспонденции: "
        "{Адрес арбитражного управляющего}), член {СРО полностью}. "
        "Определением {арбитражного суда} от {дата завершения} по делу № {номер дела} "
        "суд определил: завершить процедуру реализации имущества гражданина в отношении "
        "должника — {ФИО должника}. Освободить должника от дальнейшего исполнения "
        "имевшихся на дату обращения в суд с заявлением о банкротстве требований "
        "кредиторов, в том числе требований кредиторов, не заявленных при введении "
        "процедуры реализации имущества гражданина. Прекратить полномочия финансового "
        "управляющего — {ФИО Финансовый управляющий}. Перечислить с депозитного счёта "
        "{арбитражного суда} в пользу арбитражного управляющего — "
        "{ФИО Финансовый управляющий} — фиксированное вознаграждение в сумме 25 000 руб. "
