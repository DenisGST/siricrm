...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 5в. Маркетплейсы
        _q(p2, 2, "в) Кредиты/рассрочки на маркетплейсах", "marketplace_debts")

        # 5г. Налоги
        _q(p2, 3, "г) Неуплаченные налоги", "tax_debts")

        # 5д. Коммунальные
        _q(p2, 4, "д) Коммунальные платежи", "utility_debts")

        # 5е. Штрафы
        _q(p2, 5, "е) Штрафы", "fine_debts")

        # 5ж. По решению суда
        _q(p2, 6, "ж) Задолженности по решению суда", "court_debts")

        # 5з. Иное
        _q(p2, 7, "з) Иные задолженности", "other_debts")
