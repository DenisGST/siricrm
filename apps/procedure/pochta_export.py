...
🛑 Один лист «Лист», шапка COLUMNS, дальше строки. Порядок/имена колонок — из
официального шаблона Почты `OLD/Почта РФ/otpravka_sample …xlsx`, не переименовывать
и не переставлять: ЛК разбирает файл по позициям.
...
def build_workbook(rows: list[dict]) -> bytes:
    """xlsx для загрузки в ЛК Почты: один лист «Лист», шапка COLUMNS, дальше строки."""
