...
def _resolve_hint(reason: str, n: int) -> str:
    ...
    return {
        ...
        "debtor": "Адресат — сам должник, ФИО и адрес возьмутся из его карточки.",
        # Два режима на одном типе: без выбора — рассылка по анкете, с выбором —
        # одно письмо кредитору, о котором узнали уже в ходе процедуры.
        "creditors": ("Не выбирая адресата — уведомления всем кредиторам из анкеты. "
                      "Либо выберите одного контрагента из реестра — тогда письмо будет одно."),
    }.get(reason, "")
