"""Одноразовые коды привязки MAX-аккаунта сотрудника к его профилю.

Поток (bot + одноразовый код, как задумано для персональных уведомлений):
  1. Сотрудник в профиле жмёт «Привязать MAX» → `issue(emp_id)` кладёт короткий
     код в Redis на TTL и возвращает его для показа.
  2. Сотрудник отправляет этот код нашему MAX-боту.
  3. Входящий webhook (`apps/maxchat/processing.handle_max_event`) на каждое
     сообщение зовёт `claim(text)`; если текст = активный код — возвращает
     employee_id, код гасится, а user_id сохраняется в `Employee.max_chat_id`.
...
_CODE_KEY = "max:linkcode:code:{code}"   # code   -> employee_id
_EMP_KEY = "max:linkcode:emp:{emp_id}"   # emp_id -> code (чтобы показать в профиле)
