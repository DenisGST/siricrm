...
## kad-UI сменил submit-flow (актуально на 06.2026)

С июня 2026 поле «По делу» на главной — это tag-input компонент. Старый flow «`send_keys` → click `[alt="Найти"]`» больше не запускает AJAX-поиск (kad показывает «Укажите атрибуты дела для поиска»). Новый flow реализован в `KadSession._submit_search_form(query)`:

1. **Input лежит в tag-обёртке**: `<input placeholder="например, А50-5568/08" class="g-ph">` внутри `<div class="tag">` внутри `<div id="sug-cases" class="b-selected-tags">`. Селектор `[placeholder=...]` остался — это правильный input.
2. **Финализация tag** через одно из (после `send_keys` + `time.sleep(1.5)`):
   - Клик по `#b-suggest li.active a` — suggester-dropdown появляется через 1-2с (kad дёргает `POST /Suggest/CaseNum`).
   - Если suggester пуст — `driver.execute_script('arguments[0].blur();', inp)`.
3. **Кнопка submit — `#b-form-submit` (div-обёртка)**. Внутри есть `<button alt="Найти" type="submit">Найти</button>` — селектор `[alt="Найти"]` её **находит**, но event-handler навешан на родительский div через event-delegation. Клик по `<button>` не triggers поиск; нужно `find_element(By.CSS_SELECTOR, '#b-form-submit').click()`.
4. **`.b-case-loading`** скрывается через CSS-класс/computed-style, не inline `display: none`. Старый CSS-селектор `.b-case-loading:not([style*="display: none"])` всегда true → `_wait_search_loaded` зависал на 25с. Чек теперь через JS `getComputedStyle(el).display !== 'none'`.

> ⚠ **Под подкапотом kad жёстко детектит наш dev IP** — clicked `#b-form-submit` НЕ делает XHR (проверено через `XMLHttpRequest.prototype.send` monkey-patch). При прямом POST `/Kad/SearchInstances` через JS из той же сессии возвращается HTTP 451 «Доступ заблокирован». При прямом GET `/Card/<uuid>` — ПравоКапча (3.4КБ challenge, `id="tokenFrom"`). То есть **UI-фиксы готовы к работе как только IP откиснет** (или появится residential-прокси), но проверить на dev сейчас невозможно. Suggester работает (он публичный).
...
## IP dev в blacklist'е kad (статус на 10.06.2026)

После старых батчей parse_case (170 событий+ за раз) + 14 попыток PDF dev IP `5.35.94.218` попал в плотный blacklist у kad:
- ✅ `GET https://kad.arbitr.ru/` (главная) — 200, открывается.
- ✅ `POST /Suggest/CaseNum` — 200 (suggester работает).
- ❌ `POST /Kad/SearchInstances` — **HTTP 451 «Доступ заблокирован»** (раньше через прогрев обходилось — сейчас 451 даже после прогрева).
- ❌ `GET /Card/<uuid>` — **ПравоКапча** (3.4КБ challenge даже после прогрева; раньше прогрев снимал капчу).
- ❌ kad-JS clicked `#b-form-submit` — НЕ делает XHR (видимо kad-JS детектит наш IP и блокирует на стороне клиента).

**Без обхода парсер не работоспособен.** Варианты решения:
1. Подождать (срок откисания неизвестен, прошлый раз были сутки).
2. Residential proxy (Bright Data, SOAX, российские провайдеры) → подключить через `chrome --proxy-server=...` в `KadSession.__enter__`.
3. Расширить WireGuard split-tunnel на dev так, чтобы трафик к `kad.arbitr.ru` шёл через российский residential exit-IP.

## IP-репутация kad (исторически)
