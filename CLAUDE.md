...
- Установка агента — `tools/scan-agent/README.md`. Трей-приложение (`pystray`): индикатор + окно настроек + лог (трей→«Открыть лог», в логе строка `AGENT_BUILD` — версия) + авто-очистка `done_dir`. На сетевых папках — `PollingObserver`. Запуск без консоли — `start_agent.bat` / `start_agent.vbs` (pythonw). 🛑 «Сохранить» в настройках перечитывает только config, не код — для новой версии полный перезапуск процесса.
