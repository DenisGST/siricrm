...
**Предпросмотр в модалке** (не новая вкладка браузера; JS в `_tab_correspondence.html`): `procPreview(url, name)` — `<dialog>` с iframe (PDF/сканы/картинки — inline через `stored_download?inline=1`); `procPreviewOffice(endpoint, name)` — **doc/docx/xls** через **Microsoft Office Online Viewer** (`view.officeapps.live.com`), которому скармливается **сырой публичный pre-signed S3 URL** от эндпоинта `procedure:doc_presigned_url` (наш auth-gated `stored_download` вьюеру не годится — Microsoft качает файл сам). docx-ссылка → office-вьюер; PDF/сканы/суд.акты → inline iframe; внешние/🔒 kad-ссылки → новая вкладка.
