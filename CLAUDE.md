...
Кратко: АТС — **отдельный сервер вне docker-стека**, интеграции с CRM пока нет. AlmaLinux 9 + **Asterisk 22.8.1 из исходников** (не пакет, не FreePBX — диалплан правится руками, вокруг самописные perl/shell). За NAT: локальный `192.168.1.25`, наружу проброшены `2222→22`, **`8090→443`**, `5062/udp` (сами 80 и 443 режет провайдер). Доступы — `docs/.env_asterisk` (`600`, под `.gitignore`). Два транка: Билайн (`BEATS`) и МТТ (`MTS`); 20 внутренних номеров, часть совпадает с `Employee` в CRM. Веб-прослушка записей — **`https://pbx.siricrm.ru:8090/cdr`** (роутер пробрасывает внешний `8090` на внутренний `443`), сертификат от **своего корневого CA** — инструкция для сотрудников [`guides/pbx-cert-install.md`](./guides/pbx-cert-install.md).
...
🛑 **Свой корневой CA для веб-прослушки** (публичный сертификат тут невозможен, см. выше). **Ключ корня НЕ на АТС**, только на dev — `/var/backups/asterisk/ca/` (`700`, `ca.key` — `600`); на АТС лежит лишь сертификат сервера. Корень ограничен `nameConstraints` (critical) **только** на `pbx.siricrm.ru` + `94.233.72.30` + `192.168.1.25` — подпись для `siricrm.ru` или чужого домена отвергается (`permitted subtree violation`), так что утечка `ca.key` не даёт подделать CRM. Корень для раздачи — `docs/siricrm-pbx-ca.crt`. Оба сертификата до 20.08.2036. 🛑 У Firefox своё хранилище; на iOS мало профиля — нужно ещё включить доверие. 🛑 Перевыпуская сертификат на новое имя/IP — сперва внести его в `nameConstraints` корня. Редирект HTTP→HTTPS пока не включён: ждём, пока корень разойдётся по машинам.
