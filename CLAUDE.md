...
Секретарь сканирует бумаги на офисном МФУ; сканы попадают в лоток «Входящие сканы» (`/scans/`), где привязываются к клиенту.

- **Канал приёма — локальный агент, НЕ поллинг.** Сервер в облаке не видит офисную сеть, поэтому файлы *пушит* скрипт `tools/scan-agent/` (watchdog следит за SMB-папкой МФУ → `POST /scans/agent/intake/` с Bearer-токеном `SCAN_AGENT_TOKEN`). IMAP/беат-таска не нужны. Эндпоинт `apps/scans/agent.py` (паттерн Bearer-аутентификации как у DevOps-агента).
- **Модель `IncomingScan`** (status pending/assigned/discarded, source agent/manual) — холдинг-запись поверх `files.StoredFile`. Файл льётся в S3 prefix `scans/inbox` через общий `upload_file_to_s3`.
- **UI лотка** (`templates/scans/`): лента непривязанных сканов с inline-превью PDF (через `files:stored_download?inline=1`), ручная загрузка, модалка «Привязать»: поиск клиента + выбор папки (авто-папка `slug="scans"` «Сканы») + опц. **минимальная** форма входящей корреспонденции (услуга+контрагент+тип+дата; `Correspondence.file_link` = стабильный inline-маршрут к StoredFile). Привязка → `files.ClientFile` + (опц.) `crm.Correspondence(direction=incoming)`.
- **Доступ — флаг `Employee.can_handle_scans`** (+ admin/head_dep). Хелпер/декоратор `apps.core.permissions.can_handle_scans` / `require_can_handle_scans`. Пункт меню заведён без `requires_*`; видимость вручную фильтруется в `context_processors.sidebar_menu` по этому флагу (модель `MenuItem` своих флагов не имеет). Сидинг меню: `python manage.py scans_seed`.
- Установка агента в офисе — `tools/scan-agent/README.md` (NSSM/systemd, `config.ini`).
...
Генерация документов по .docx-шаблонам с плейсхолдерами `{key}`. Первый реализованный документ — **договор юруслуг БФЛ** (кнопка «Составить договор» в модалке услуги).
...
- **Дальше по ТЗ (не сделано):** генераторы исков/ходатайств/запросов, выбор контрагента и адреса, отправка по email, контроль ответов, заказные письма Почтой РФ + трекинг РПО.
