...
- Установка агента в офисе — `tools/scan-agent/README.md` (NSSM/systemd, `config.ini`). Трей-приложение (`pystray`): индикатор состояния + окно настроек + лог + авто-очистка `done_dir`. На сетевых папках — `PollingObserver` (события ФС по SMB не приходят).
