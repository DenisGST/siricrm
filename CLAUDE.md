...
apps/callcenter     — рабочее место оператора колл-центра: канбан обзвона, автонаполнение из звонков/лидов, чёрный список номеров, результаты звонков (см. ниже)
apps/reports        — раздел «Отчёты»: аналитика/отчётность (вкладки), 1-й отчёт «Отдел продаж» + бюджет ОП (см. ниже)
apps/wiki           — руководство пользователя `/wiki/` (дерево статей в БД, markdown, правка в UI, сид `wiki_seed`; см. ниже)
...
**`ssh siri-prod` с нового dev работает** (проверено 27.08.2026 — ключ в `authorized_keys` прода добавлен; прежняя пометка «не работает» устарела). Удобно для того, чего не умеет DevOps-агент: у него нет обработчика «перезапусти контейнер», а `pbx-listener`/`arbitr-runner` после деплоя надо трогать руками. Сам деплой и мониторинг SSH не требуют — идут по HTTPS через агента с токеном.
...
- **Telegram webhook'и не работают на наших серверах** — split-tunnel заворачивает ответный SYN-ACK на входящие от Telegram обратно в туннель, Telegram видит `Connection timed out`. Для приёма обновлений используем **polling** (`getUpdates`) через Celery beat — `apps/telegram/tasks.py:poll_telegram_leads` каждые 10с с long-poll timeout=20с и SETNX-локом `cache.add('telegram_leads:poll_lock')` (иначе параллельные таски ловят 409 Conflict). Переключать polling, не трогая код, — через `PeriodicTask` в БД, но 🛑 **только `obj.save()`, не `.update()` по queryset'у**: django-celery-beat узнаёт об изменении расписания по сигналу `post_save` (он двигает метку `PeriodicTasks.last_update`), а `.update()` сигналов не шлёт — beat молча продолжает работать по старому расписанию, пока его не перезапустят. Обжигались 27.08.2026: приём лидов был «включён» в БД и 4 минуты не запускался. Правильно: `t = PeriodicTask.objects.get(name='poll-telegram-leads'); t.enabled = False; t.save()`.
...
🛑 **Кто именно поллит бота — по серверам разное** (`getUpdates` на один токен из двух мест = 409 Conflict): на **проде с 27.08.2026 работает `poll-telegram-leads`** (канал лидов `-1004426367072` «Банкротство физических лиц с юридическим сопровождением», бот @SiriusCRMBot в нём администратор), а `poll-telegram-bot-private` там **намеренно выключен**: `_poll_once` одной задачей ловит и посты канала, и личку, поэтому коды привязки продолжают работать. На **dev** наоборот: `poll-telegram-leads` выключен, а личку ловит **`telegram.poll_bot_private`** (`poll-telegram-bot-private`, раз в 10с, только личка — лиды не создаёт); тот же токен там занят ботом мониторинга (`MONITOR_BOT_POLL=true`), поэтому `poll_bot_private` там сам уступает (`skipped: monitor_bot_owns_polling`), а код привязки ловит `_handle_monitor_update` (проверка кода — ДО allowlist'а, код сам себе пароль). Если понадобятся и лиды, и привязка одним поллером — включать только `poll-telegram-leads`: его `_poll_once` умеет обе ветки. 🛑 Бот не пишет первым — но привязка и есть первое сообщение боту, так что у привязанных диалог уже открыт.
...
**Полное описание:** [`asterisk_integration_claude.md`](./asterisk_integration_claude.md) — устройство АТС, транки, IVR и диалплан, записи разговоров, схема CDR, точки для интеграции с CRM, безопасность, шпаргалка команд.

Кратко: АТС — **отдельный сервер вне docker-стека**, интеграция с CRM живая (журнал звонков, записи, звонок по клику, всплывашка). AlmaLinux 9 + **Asterisk 22.8.1 из исходников** (не пакет, не FreePBX — диалплан правится руками, вокруг самописные perl/shell). За NAT: локальный `192.168.1.25`, наружу проброшены только `2222→22` (SSH) и `5062/udp` (SIP) — 🛑 сами 80 и 443 режет провайдер, а веб-прослушка закрыта 23.08.2026. Доступы — `docs/.env_asterisk` (`600`, под `.gitignore`). Два транка: Билайн (`BEATS`) и МТТ (`MTS`); 20 внутренних номеров, часть совпадает с `Employee` в CRM. **Веб-прослушка на АТС закрыта 23.08.2026** — записи слушают в CRM, раздел «Звонки»; снаружи у АТС остались только SSH и SIP.

🛑 **Готовность к интеграции неравномерная**: **CDR и записи читаются уже сейчас** без правок на АТС (MariaDB, таблица `cdr` с полем `rec_name` = путь к .wav; сами файлы в `/records`, 13.4k / 102 ГБ). А вот **AMI выключен**, **ARI — `read_only` и только с `127.0.0.1`** → карточка клиента при входящем и звонок из CRM в один клик потребуют правок на самой АТС.

🛑 **`pjsip.conf` генерируется из таблицы `users`** скриптом `/root/pjupd.pl` — правка конфига руками живёт только до следующего запуска скрипта. Источники — `/etc/asterisk/.pjsip_{global,extreg,templates}.temp`, править ТОЛЬКО их (24.08.2026 они разошлись с правленым руками конфигом, и первый же `pjupd.pl` откатил фиксы транков от 20.07 — `max_retries`, `expiration`, и раскомментировал опасный глобальный `[acl]`). NAT-настройки абонентов (`rewrite_contact`, `qualify_frequency`) — там же, в шаблонах.

🛑 **Доступность портов АТС снаружи упирается в ДВА независимых фильтра** (не перепутать): **провайдер режет входящие 80/443** на DSL-линии → снаружи **таймаут**, лечится только нестандартным внешним портом на роутере; и **хостовый iptables отбивает всё явно не разрешённое** → **отказ RST**, при этом `policy ACCEPT` обманчива — последним правилом стоит `REJECT all`, а `ACCEPT all` в середине цепочки это `-i lo` (интерфейс в `iptables -L -n` не виден), из-за чего локальный `curl` проходит, а снаружи отказ. **Открывая новый порт** (AMI 5038, MySQL для CRM) — `iptables -I INPUT <перед REJECT> -p tcp --dport <порт> -j ACCEPT` + `iptables-save > /etc/sysconfig/iptables`. 🛑 Let's Encrypt здесь невозможен в принципе: HTTP-01 проверяется только по 80, TLS-ALPN-01 только по 443, нестандартные порты LE не принимает — остаётся DNS-01. 🛑 Записи в `access_log` НЕ доказывают, что порт открыт.

🛑 **Перенос звонков в CRM — `apps/telephony` + `tools/pbx-agent/`** (на проде с 23.08.2026: 12.9k звонков, 8345 записей / 4.2 ГБ в S3). Поток **push с АТС**: `cdr` + `/records/*.wav` → агент (`lame -b 24`, сжатие ~5×) → HTTPS с Bearer `PBX_AGENT_TOKEN` → `apps.telephony.agent` → S3 (`telephony/records`) + модель `Call`. 🛑 **Ключей S3 на АТС нет** — файл принимает CRM, она же кладёт в бакет (на АТС открыт root по паролю). Состояние агента — SQLite на АТС, водяной знак по `cdr.id`; таблицы АТС не трогаем. 🛑 wav <1 КБ не переносится (у неотвеченного звонка файл 44 байта), файл моложе 60 с не берётся (MixMonitor дописывает), wav удаляется через **10 дней** и только после подтверждённой отправки. Направление звонка определяется **по номерам**, а не по контекстам диалплана; у входящих внешний номер в `src` (в `dst` мусор). Клиент — через `find_client_by_phone`, сотрудник — по имени в `clid` самого звонка, и лишь затем по `Employee.sip_extension`. 🛑 **Внутренние номера переходят от человека к человеку** (502: Попова в феврале 2026 → Кудинов в августе), поэтому привязка «по текущему владельцу» задним числом приписала бы чужие разговоры. `map_extensions --apply`: текущий владелец = имя из самого свежего звонка; каждый звонок — по своему `clid`; для входящих (там в `clid` номер клиента, а не имя) строится **шкала владения номером** и берётся владелец на дату звонка. 🛑 **Звонок по клику и всплывашка** — `apps/telephony/ami.py` (клиент AMI написан вручную: новая зависимость означала бы `rebuild` вместо `deploy`). Звонок: `POST /telephony/call/` → `Originate` поднимает трубку сотрудника, затем АТС набирает клиента. 🛑 `CallerID` обязателен — диалплан выбирает транк ПО НОМЕРУ звонящего; номер приводим к `8XXXXXXXXXX` (так записаны все реальные исходящие в CDR). Всплывашка — отдельный контейнер **`pbx-listener`** (`manage.py pbx_ami_listen`), долгоживущий AMI: `DialBegin` → карточка звонящего владельцу номера, `DialEnd` → только перекрашивает её в «Пропущенный». 🛑 Карточка **не убирается сама** и живёт в базе (`IncomingCallAlert`): смысл в том, что человек отошёл и должен увидеть пропущенный; стек восстанавливается при загрузке страницы, убирает только сотрудник крестиком. Показываются сегодняшние — к новому дню список обнуляется сам. 🛑 Ключ карточки — «нога звонка + внутренний номер», иначе при параллельном обзвоне 201&202 ответ одного гасит карточку у второго. 🛑 `restart: on-failure` — без `PBX_AMI_HOST` команда штатно выходит с кодом 0, и `unless-stopped` крутил бы контейнер вечно. 🛑 **`deploy` НЕ перезапускает `pbx-listener`** (как и `arbitr-runner`) — после выкатки нужен `docker restart siricrm-pbx-listener-1`. Репозиторий примонтирован волюмом (`.:/app`), поэтому для нового КОДА хватает обычного `restart`; `up -d --force-recreate` нужен, только если поменялся env. 🛑 **В проекте нет корневого логгера** (`LOGGING["loggers"]` = django/celery/userbot/maxbot), поэтому `logger.info` из `apps.*` не виден нигде — пустой лог ничего не доказывает; слушатель пишет через `self.stdout`. **Канал — WireGuard-туннель прод↔АТС** (`10.77.0.1↔10.77.0.2`), AMI слушает только его: пароль и события идут открытым текстом.

🛑 **Поиск по номеру: один номер лежит в трёх видах** — `src`/`dst` как набрали на АТС (`89537741564`, восьмёрка), `counterparty_phone` нормализованный (`79537741564`, семёрка). Искать надо по ЦИФРАМ запроса И по варианту без ведущей 7/8, подстрокой по всем трём полям — иначе `+7953` не находит ничего. **Раздел «Звонки»** `/telephony/` — журнал с фильтрами. Права: **свои звонки слушает каждый сотрудник** (по `Employee.sip_extension`), **чужие — только руководство** (`admin`/`head_dep`/`managing_partner`) и те, кому выдан флаг **`Employee.can_listen_calls`** (смысл флага изменился: теперь это доступ к ЧУЖИМ записям). 🛑 Ограничение на queryset'е, а не в шаблоне — иначе через `?page=` рядовой вычитал бы чужую клиентскую активность; право на конкретный звонок проверяется и в `call_recording` (ссылку зовут по id мимо журнала). 🛑 Звонок без `employee` (входящие на группу — в CDR `dst`=`1`/`s`) видит только руководство: на проде ~1.9k из 12.9k. 🛑 **Каждое прослушивание пишется в `CallListen`**; ссылка на mp3 выдаётся только вьюхой `call_recording` (presigned 10 мин, inline+audio/mpeg) — прямых ссылок на S3 в шаблонах быть не должно, иначе прослушивание не попадёт в журнал. 🛑 `playCallRecording`/`resetCallFilters` — глобально в `dashboard.html`, не в партиале (грабля `procPreview`); плеер — `<dialog>`+`showModal()`. Подробно — [`asterisk_integration_claude.md`](./asterisk_integration_claude.md).

🛑 **Пропущенные звонки и голосовые переведены с почты в CRM** (25.08.2026) — вкладка «Пропущенные» в разделе «Звонки» (`MissedCall` + справочник `CallGroup`). Причина замены: почта с АТС **не уходит вообще** — postfix релеит через `smtp.mail.ru`, а тот на тарифе для бизнеса отвечает `535 Net dostupa na vashem tarife`: ~19k отказов SASL в неделю и ни одной успешной внешней доставки за месяц (опечатка `rop@antydiolg.ru` была лишь вторым слоем). Потери — ~190 обращений в месяц (110 пропущено / 79 голосовых / 42 брошено в IVR против 16 состоявшихся разговоров). 🛑 **Источников записи два**: диалплан (`sc_notify_crm.sh` из `miss_call_*`/`mess_rec_*` → `POST /telephony/agent/missed/`) даёт мгновенное уведомление, выгрузка CDR — страховку и единственный путь для брошенных в IVR (их обрывает `Read`, а не `Dial`); ключ идемпотентности — `linkedid`, метка `notified_at` не даёт разослать дважды. 🛑 **Голосовой wav шлёт агент, а не диалплан** (MixMonitor дописывает файл — отправленное сразу было бы обрезком): `/var/spool/asterisk/monitor` → mp3 → S3. 🛑 **Бэкфилл не сыплет в реестр** — запись заводится по звонкам не старше 48 ч, уведомление шлётся не старше часа. Получатели — справочник `CallGroup` (отдел ∪ подписчики ∪ по флагу руководство ∪ владелец номера), каналы Telegram + MAX + карточка в интерфейсе + событийка клиента (`call_missed`); личный отказ — `Employee.notify_missed_calls` в профиле (деф. ВКЛ). 🛑 **Отделы у групп после выкатки пустые** — проставить руками, иначе уведомления идут только руководству. Автозакрытие: состоялся разговор с номером → открытые обращения закрываются сами. 🛑 Реестр — вкладка ВНУТРИ `/telephony/?tab=missed`, а не отдельный раздел: `?open=` принимает только адреса пунктов меню. **На проде с 25.08.2026** (диалплан пропатчен `tools/pbx-agent/patch_dialplan.py`, 7 мест, `dialplan reload`). 🛑 **Почта на АТС выключена совсем**: postfix погашен и снят с автозапуска, очередь вычищена (62 письма), старые почтовые скрипты заглушены `exit 0`, и 🛑 **снят cron `post_restart.sh`**, который рестартовал postfix каждые 4 часа и молча воскресил бы его. 🛑 Заглушены именно скрипты, а не только транспорт: при погашенном postfix `mutt` всё равно кладёт письмо в maildrop, и очередь тихо росла бы.

🛑 **Осознанно оставлено владельцем** (23.08.2026, риски приняты, самовольно не «чинить»): вход root по паролю с любого IP; единый SIP-пароль на всех 20 номерах при ACL `acl-endpoints`, который фактически разрешает всё (`deny 0/0` затем `permit 0/0` — выигрывает последнее правило) → главный риск тарифного мошенничества. **Отложено:** MariaDB 10.5→10.11 (мажорная версия) и перезагрузка ради нового ядра. Память: `asterisk-pbx`, `asterisk-security-2026-08`.

## Колл-центр — рабочее место оператора (apps/callcenter)

**Полное описание:** [`call-center.md`](./call-center.md) — модели, доска и колонки, автонаполнение (обе точки подключения + предохранители), чёрный список, следующее действие, результат звонка, панель управления, деплой, что не проверено вживую.

Кратко (**на проде с 27.08.2026**): раздел `/callcenter/` — канбан обзвона. **Колонки — ДАННЫЕ** (`CallCenterColumn`), правятся в Панели управления → вкладка «Колл-центр» (под-вкладки Колонки · Результаты звонков · Чёрный список); вёрстка не знает ни их числа, ни имён. Карточка — отдельная модель `CallCenterCard` (O2O к `Client`), а не поле на клиенте: статус клиента на главном канбане от обзвона ездить не должен. **Владелец назначается только вручную** («взять»); чужую перехватывает и освобождает лишь руководство. Селектор «чей канбан»: Все / Мои / Свободные, руководителю (`is_management`) — канбан любого оператора (зеркало `?viewed_employee=` из «Моего канбана»).

**Автонаполнение** (`intake.py`): входящий с неизвестного номера → заводится неидентифицированный клиент + карточка; лид из TG-канала → карточка. 🛑 **Колонка-приёмник задаётся флагом на самой колонке** (`catch_unknown_calls`/`catch_telegram_leads`), не по имени и не в settings — иначе переименование в панели ломало бы приём; флаг он же выключатель источника. Точек подключения по звонкам ДВЕ, обе идемпотентны: слушатель AMI (пока телефон звонит) и CDR-приём (`upsert_call`, страховка — до трубки доходит не всякий звонок). 🛑 Предохранители: звонок старше 48 ч карточку не создаёт (бэкфилл 13 тыс. CDR завалил бы доску), а при параллельном обзвоне проверяется ВЛАДЕЛЕЦ `ClientPhone`, а не «не None» — `add_client_phone` при конфликте отдаёт запись победителя, и без проверки проигравшие ноги оставляли клиентов-сирот.

**Чёрный список** (`BlockedPhone`) режет только автонаполнение — звонок остаётся в журнале и реестре пропущенных. 🛑 Номер хранится нормализованным, иначе ловился бы только тот формат, в котором его завели. Кнопка 🚫 на карточке заносит номера клиента и ставит автозаведённому пустому клиенту статус «На удаление».

**Следующее действие** на карточке (суть + дата/время + автор) — основа под напоминалку; доставку строить на существующем `notifications.Notification` + beat `revive_snoozed`, второй механизм не заводить. 🛑 `datetime-local` рендерить строго `|date:'Y-m-d\TH:i'` и привязывать наивное время к `Europe/Moscow`.

**Результат звонка** (`CallResult` + `CallOutcome`): после входящего И исходящего у оператора всплывает модалка (плитки результатов, комментарий, план следующего шага); «Позже» оставляет звонок в счётчике «Без результата». 🛑 Запись заводится ДО появления `telephony.Call` — CDR приезжает позже, поэтому ключ это нога звонка, а ссылка на `Call` проставляется потом. 🛑 `_on_dial_end` теперь ловит и ИСХОДЯЩИЕ — по внутреннему номеру в `Channel`, а не только в `DestChannel`.

**Права:** раздел — superuser ∪ `is_management` ∪ флаг `Employee.can_access_callcenter`; 🛑 **модалка результата — строго по флагу** (через широкую проверку она всплывала бы у всего руководства после каждого разговора). 🛑 Гейт по роли `operator` был бы дырой: это значение по умолчанию у всех, кому роль не меняли. 🛑 `Client.objects.visible_to` + `crm/rules.py` дополнены узким правилом «клиент на доске виден её оператору» — без него доска у рядового пуста (у клиента из звонка нет ни ответственного, ни услуг); `Service.visible_to` намеренно не трогали.

🛑 **Деплой:** зависимостей нет → обычный `deploy`, но он **НЕ перезапускает `pbx-listener`** — после выкатки нужен `docker restart siricrm-pbx-listener-1` (репо примонтировано волюмом, `--force-recreate` нужен только при смене env). Иначе ветка AMI останется на старом коде. 🛑 Ветка AMI **не проверена вживую** — на dev нет ни АТС, ни контейнера.
...
Канбан-колонка (`PAGE_SIZE=12`, intersect-подгрузка, `#id`-root), `#global-progress` (watchdog от «вечной» полосы), `client_edit` без reload, файловый менеджер (office/PDF inline-превью), глобальный поиск, `showToast()`, кэш статики (`STORAGES` обязателен), `{% comment %}` для multiline, чат-модалка (ленивый список, `_activeTelegramClientId`, `?pin_client_id`). 🛑 **Разделы отдают голый партиал при прямом заходе** (без `<html>`/tailwind/htmx) — выглядит как сломанная страница, где ничего не работает. Дашборд умеет `?open=/telephony/` (автозагрузка в `#content-area`), раздел при не-HTMX заходе редиректит туда сам; значение `?open=` сверяется со списком пунктов меню (`_resolve_open_section`), иначе через ссылку можно дёрнуть любой внутренний эндпоинт. Пока сделано только «Звонкам». Подробно: **[`docs/ui-conventions.md`](./docs/ui-conventions.md)**. Виджеты-счётчики дашборда + 🛑 гочча наследования `hx-target` (затирала `<body>`) — **[`docs/dashboard-widgets.md`](./docs/dashboard-widgets.md)**.
...
Кратко: раздел `/accounting/` (право `can_access_accounting` — роль `accountant`/admin/руководство). Входящие платежи из двух источников → очередь «Уведомления» (статус `не привязан`→`привязан`/`неопознанный`), бухгалтер вручную привязывает к клиенту и начислениям (разбивка суммы) → создаётся `finance.Payment`, гасится `Charge`. **Выписка р/с ТБанк** — поллинг `GET business.tbank.ru/openapi/api/v1/statement` (beat `accounting-poll-statement`, throttle 3ч); эквайринг-зачисления (плательщик «АО ТБанк», ИНН 7710140679) помечаются `is_settlement` и непривязываемы. **Эквайринг** — приём вебхуком `/accounting/acquiring/webhook/` (🛑 подпись: булево как `"true"/"false"`; ТБанк НЕ отдаёт введённые ФИО/телефон → берём из prepay `/accounting/acquiring/prepay/`, страница `fo-y.ru` шлёт их `sendBeacon`'ом по `OrderId`). 🛑 Деплой при смене env: deploy делает `restart` (env не перечитывает) → нужен `up --force-recreate`. 🛑 **Оплата с сайта — серверный Init, а НЕ JS-виджет ТБанка** (фикс 20–21.08.2026, на проде): виджет `securepay.tinkoff.ru/.../tinkoff_v2.js` отдаётся с сертификатом Russian Trusted CA (Минцифры) → в браузере без этого корня скрипт блокируется (`ERR_CERT_AUTHORITY_INVALID`), `window.pay` = `undefined`, `tbankPay()` падает с `ReferenceError` **молча**; beacon prepay уходит ДО `pay()`, поэтому в CRM это видно как всплеск `AcquiringPrepay` при нуле платежей (17.08: 105 попыток от 5 человек → 2 оплаты). Перехостить виджет нельзя — `HOST_URL` зашит в его код. Теперь форма fo-y.ru шлёт POST на `/accounting/acquiring/pay/` → `views.acquiring_pay` (csrf-exempt, публичный) → `integrations.acquiring_init` (**`securepay.tbank.ru`**, Let's Encrypt; подпись прежним `acquiring_token`, 🛑 `DATA` в подпись не входит) → `302` на `pay.tbank.ru`. Ошибка → `templates/accounting/pay_error.html`. `OrderId` остался `FOY-<ms>-<rnd>` — склейка prepay/нотификации/реестра не менялась, вебхук не трогали. Настройка `TBANK_ACQUIRING_API_BASE` (дефолт `securepay.tbank.ru/v2`) + опц. `TBANK_ACQUIRING_SUCCESS_URL`/`_FAIL_URL`. Блок для страницы сайта — `docs/foy-payment-form.html`. 🛑 **Гочча SP Page Builder (Joomla)**: страница лежит в БД в ДВУХ колонках `nq1gm_sppagebuilder` — `content` (структура, из неё рендерится фронт) и `text` (скомпилированный); правка через админку может не сохраниться незаметно — редактор показывает новое, сайт отдаёт старое. Проявилось как «на десктопе платится, с телефона нет» (у владельца браузер доверял корню Минцифры). Проверять только фактом: `curl -s https://fo-y.ru/index.php/oplata-uslug-tinkoff | grep -c acquiring/pay` → 1. 🛑 Сайт fo-y.ru был **скомпрометирован** (JS-инъекция + веб-шеллы с RCE) — разбор и что осталось сделать: `docs/foy-injection-2026-08.md`. 🛑 **Токен T-API привязан к IP** — у прод-сервера 5 публичных IP, перезагрузка меняет primary egress → 403 `FORBIDDEN`; лечится перевыпуском токена с вайтлистом ВСЕХ прод-IP (или без ограничения). 🛑 **Роль `accountant`** = рабочее место бухгалтера: finance-CRUD (кроме удаления) + видит ВСЕХ клиентов/услуги (`visible_to`/`can_view_all_clients`/`rules.py` — менять синхронно). На dev поллинг выписки выключен. Боевая интеграция живая на проде (проверена реальным платежом). Подробно: файл выше + память `tbank-payments-integration-plan`.
...
**Правка записи + вложения + комментарий (21.08.2026).** Клик по строке в любой из трёх таблиц («Запросы», «Входящие», «Исходящие») открывает **карточку записи** — модалку с правкой всех параметров: запрос (`procedure:request_card`/`_save`) — тип/название/адресат/исх.№/статус/способ и дата отправки/срок/данные ответа; письмо (`procedure:correspondence_card`/`_save`) — раздел (можно перекинуть входящее↔исходящее), тип письма, контрагент, №/дата, блок контроля ответа, связь с запросом, замена основного скана. 🛑 Обработчик клика — **`data-edit-url` на `<tr>` + делегирование на `document` в `dashboard.html`** (общий на все таблицы, переживает перезагрузку партиала); клики по `a/button/input/label/select/textarea` отсекаются, иначе кнопки действий и чекбоксы открывали бы карточку.

**Текст документа — прямо в карточке запроса**: блок во всю ширину под колонками, **ленивый** (`procedure:request_text_block`, разворот по кнопке → `?collapsed=1` сворачивает) — 🛑 абзацы тянутся из S3 и парсятся python-docx, грузить их на каждый клик по строке нельзя. Сохранение (`request_text_save`) пересобирает .docx→PDF и перерисовывает блок, карточка остаётся открытой; отдельная модалка правки на иконке ✎ в строке таблицы осталась как была (общие хелперы `_doc_paragraphs`/`_posted_paragraphs`/`_save_doc_text`). 🛑 Недоступный в S3 файл (документ в прод-бакете, а мы на dev) даёт понятное сообщение, а не 500 (`_doc_paragraphs_safe`).

**Комментарий** — одно поле на запись (`Request.notes` / `Correspondence.comments`, обе переименованы в «Комментарий»), правится в карточке и выводится под названием строки таблицы с иконкой 💬. Ленты с авторами нет — сознательно.

**Доп. вложения** — модель **`procedure.DocumentAttachment`** (generic FK: `content_type`+`object_id`, `GenericRelation attachments` на `Request` и на `crm.Correspondence`, S3 prefix `procedure/attachments`). Основные слоты (`document_pdf`/`_docx`/`response_scan`/`stored_file`) не тронуты — вложения это всё сверх них (приложение к ответу, опись, конверт с отметкой). Одна пара вью на обе таблицы: `procedure:attachments_add`/`attachment_delete` с `target ∈ {request, correspondence}`. В таблицах — бейдж 📎N (счётчик одним запросом, `_attach_counts`, не N+1). 🛑 Блок вложений в модалке лежит **ВНЕ `<form>`** (кнопка «Сохранить» связана через `form=`): htmx при не-GET подмешивает значения ближайшей формы, и выбранные файлы уезжали бы повторно на каждое сохранение. 🛑 Удаление вложения сносит только строку — объект в S3 остаётся (как в остальных разделах); удаление запроса/письма сносит вложения каскадом через `GenericRelation`. Миграции `procedure/0023`, `crm/0102`, деплой обычный. **На проде с 21.08.2026.**
...
## Руководство пользователя — вики `/wiki/` (apps/wiki)

Пользовательская документация ВНУТРИ CRM: `https://<env>/wiki/` (иконка «?» в шапке, открывается **отдельной вкладкой** — свои страницы-шеллы, а не HTMX-своп в `#content-area`). Дерево статей `WikiArticle` (самоссылка `parent` + `order`), тело — markdown, у каждой статьи свой URL `/wiki/a/<slug>/` (кириллица транслитерируется). Нумерация «1 / 1.1» НЕ хранится — считается из позиции узла (`services.build_tree`). Читают все авторизованные, правят — `is_references_access` (`permissions.can_edit_wiki`): в UI кнопки «Править»/«+ Раздел»/удалить/↑↓, живой предпросмотр и поиск по дереву.

- **Рендер** — `markdown_render.render_markdown`: расширения `extra` (таблицы) + `sane_lists` + **`nl2br`** + `admonition` + `toc`, затем **bleach-санитайз** (без script/style/iframe/form). Врезки — `!!! note "Заголовок"` (стили есть у `note`/`warning`/`danger`, см. `templates/wiki/base.html`); внешние ссылки автоматически `target=_blank`.
- 🛑 **`nl2br`: каждый перенос строки → `<br>`** — в `seed_content.py` абзац пишется ОДНОЙ физической строкой (не переносить по ширине!), блоки разделяются пустой строкой. Иначе текст рвётся посреди предложения.
- 🛑 **Контент живёт в БД и деплоем НЕ едет.** Сид `apps/wiki/seed_content.py` (дерево как данные) → `python manage.py wiki_seed` — гонять на КАЖДОМ сервере вручную, как `procedure_seed`/`efrsb_seed`. Идемпотентно: место в дереве синхронизирует всегда, текст создаёт только у отсутствующих. `--force-text` перезаписывает тексты из кода (затрёт правки из UI!), `--only SLUG` (можно повторять) ограничивает действие — существующие статьи вне списка не трогаются вовсе.
- **Разделы на 27.08.2026** (18 статей): 1 О SiriCRM (+ разработчик, для кого) · 2 Вход и регистрация · 3 Основной канбан · 4 Чат с клиентами · 5 График консультаций · 6 Звонки (+ журнал, пропущенные, звонок из CRM, карточка «вам звонили») · 7 Колл-центр (+ доска, результат звонка, следующее действие, настройка) · 8 Входящие сканы (+ 3). **Не описаны:** клиенты/услуги/канбаны услуг, файлы, анкеты, поиск, уведомления, профиль, Юрист БФЛ, арбитраж, финансы, бухгалтерия, АФД, отчёты, админ-раздел.
- 🛑 Ссылки между статьями — абсолютным путём `/wiki/a/<slug>/`, не относительным.
...
- HTMX-партиалы — в `templates/<app>/partials/`. daisyUI 4 классы, Tailwind pre-compiled (`static/css/tailwind.css` — не пересобирать без необходимости). Если добавил новые классы и нужна пересборка (Node на серверах нет): `docker run --rm -v "$(pwd)":/app -w /app node:20-alpine sh -c "npm install && npm run build"`, затем restart `web` (он сам сделает `collectstatic`). 🛑 **Перед пересборкой сохранить копию и после сверить списки классов**: классы, собираемые в JS динамически (`showToast` через `alert-${type}`), сканер не видит и они молча пропадут — сломав вёрстку там, где никто не трогал. Процедура и результат проверки 23.08.2026 — в [`docs/ui-conventions.md`](./docs/ui-conventions.md). `node_modules/` в `.gitignore` — коммитить только `tailwind.css`.
...
- [`asterisk_integration_claude.md`](./asterisk_integration_claude.md) — IP-телефония: сервер Asterisk (транки Билайн/МТТ, IVR, записи разговоров, CDR в MariaDB, AMI/ARI, безопасность)

Внутренние модули (детально):
- [`call-center.md`](./call-center.md) — рабочее место оператора колл-центра (`apps/callcenter`): доска и колонки, автонаполнение из звонков/лидов, чёрный список, следующее действие, результат звонка
...
- [`docs/foy-injection-2026-08.md`](./docs/foy-injection-2026-08.md) — компрометация сайта fo-y.ru (JS-инъекция `sflog.ru` + веб-шеллы с RCE через загрузку иконок SP Page Builder): разбор, что вычищено 21.08, что осталось сделать владельцу
- `docs/foy-payment-form.html` — готовый блок формы оплаты для страницы fo-y.ru (POST на `/accounting/acquiring/pay/`)
