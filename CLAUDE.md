...
Тонкий слой над `apps/procedure` + `apps/afd`. Вкладка **«Публикации»** в карточке дела БФЛ → под-вкладки **ЕФРСБ** (реализована) и **КоммерсантЪ** (заглушка, отдельная ветка). На dev и проде с 24.06.2026, но **СПЯЩИЙ** (`EFRSB_ENABLED=False`, кредов нет → клиент «не настроен», beat `disabled`; `efrsb_seed` на проде НЕ прогнан — каталог пуст до согласования с АУ).
