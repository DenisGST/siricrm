...
- **VPN (Amnezia/WireGuard)** на обоих серверах — split-tunnel: через VPN идёт только трафик к Telegram-подсетям (`149.154.160.0/20`, `91.108.4.0/22`…`91.108.56.0/22`) и Anthropic (`160.79.104.10/32`, `34.36.57.103/32`), остальное напрямую. 🛑 Через `awg-quick`/`wg-quick` **не ставить `AllowedIPs=0.0.0.0/0` без `Table=off`** — иначе уведёт дефолтный маршрут и порвёт SSH. (Канал `awg1` на dev специально использует `AllowedIPs=0.0.0.0/0` + `Table=off` + ручной policy-routing — это безопасно, дефолт не трогается; см. ниже.)
  - prod (обновлено 30.06.2026): **те же два канала + failover что на dev, схема идентична.**
    - `claude0` (старый, плоский WG) — `/etc/wireguard/claude0.conf`, peer `72.56.73.137:37539`, systemd `wg-quick@claude0`. Подсети Telegram/Anthropic — прямо в его `AllowedIPs` → маршруты в `main`. 🛑 На 30.06.2026 **peer не отвечает 12+ часов** (handshake `12h ago`, `ping -I claude0` молчит) — backup-роль в failover нерабочая, надо решить: восстановить endpoint у провайдера или снести. Подождать сутки, потом принять.
    - `awg1` (новый, основной) — AmneziaWG, `/etc/amnezia/amneziawg/awg1.conf`, peer `130.17.1.69:46161` (тот же endpoint что на dev), `Address = 10.8.1.7/32`, systemd `awg-quick@awg1` (enabled). Конфиг — копия dev: `Table=off` + `MTU=1376` + MSS-clamp + PostUp/PostDown с `ip route replace default dev awg1 table 200` + 8 `ip rule add to <подсеть> lookup 200 priority 100`. DNS из сырого конфига провайдера **намеренно не перенесён** — иначе awg-quick через resolvconf переписал бы наш кастомный `/etc/resolv.conf` с DNS-fallback. На 30.06.2026 живой, handshake свежий.
    - **авто-failover**: `awg-failover.service` (`/usr/local/sbin/awg-failover.sh`) с `PRIMARY=awg1 BACKUP=claude0`. Логика та же что dev (15с тиков, threshold 2, не переключается на дохлый). Лог: `journalctl -t awg-failover`.
    - **Установка amneziawg-пакета на проде** (нет в стандартных репах): `add-apt-repository -y ppa:amnezia/ppa && apt install -y amneziawg amneziawg-tools` — встанет userspace utility + DKMS-модуль ядра.
  - dev (обновлено 30.06.2026): **два независимых Amnezia-канала + авто-failover.**
    - `awg0` (старый, нестабильный) — AmneziaWG, `/etc/amnezia/amneziawg/awg0.conf`, peer `72.56.73.137:33886`, systemd `awg-quick@awg0`. Подсети прописаны прямо в его `AllowedIPs` → маршруты уходят в таблицу `main`. 🛑 Часто мёртв (30.06.2026 был без handshake 12+ часов) — именно поэтому добавлен awg1.
    - `awg1` (новый, основной) — AmneziaWG, `/etc/amnezia/amneziawg/awg1.conf`, peer `130.17.1.69:46161`, systemd `awg-quick@awg1` (enabled). Поднят с `Table=off` + `MTU=1376` + MSS-clamp (как awg0). Маршрутизация вынесена в `PostUp/PostDown` конфига: таблица `200` (`default dev awg1`) + по одному `ip rule add to <подсеть> lookup 200 priority 100` на те же Telegram/Anthropic подсети. Приоритет `100` < `main` (32766) → перекрывает awg0; снять правила / `stop` сервиса → мгновенный откат на awg0.
    - **авто-failover**: `awg-failover.service` (`/usr/local/sbin/awg-failover.sh`, `BindsTo`/`WantedBy`=`awg-quick@awg1`). Каждые 15с пробит ОБА туннеля напрямую (`curl --interface awgX` к Telegram/Anthropic) и переставляет `default` в таблице `200` на здоровый, приоритет awg1→awg0; если оба мертвы — остаётся на текущем (не переключается на дохлый). Гистерезис 2 цикла. Лог: `journalctl -t awg-failover`.
    - Управление: откат на старый VPN — `systemctl stop awg-quick@awg1` (failover останавливается вместе с ним, трафик возвращается на awg0/`main`); вернуть — `systemctl start awg-quick@awg1`. Проверка: `ip rule show`, `ip route show table 200`, `awg show awg1`. Все правила переживают ребут (зашиты в Post-Up/Down + `enable`d).
...
### MAX: персональные уведомления о судебных событиях (парсер kad → сотрудник)

Парсер kad при **новых записях/файлах** дела шлёт непустое уведомление в MAX каждому сотруднику с `Employee.notify_court_events_max=True` и привязанным `Employee.max_chat_id` (`is_active=True`). Пустые парсинги молчат. Формат (**MAX markdown**, `send_max_message(..., text_format="markdown")`): `[номер дела](kad-карточка) ФИО` / `Новая запись (дата): {тип акта}` / `{суть}` / `Новый файл: [name](kad-файл)`. Билдер — `apps/arbitr/notifications.py:_build_court_event_text` + `send_court_event_notifications`; вызов из `_kad_smart_one` и ручного `kad_monitor_one_case` (идемпотентно — событие «новое» лишь при первом обнаружении). Детализация новых записей отдаётся из `_persist_case_info` (`new_events_detail`).

🛑 **Семантика полей `ArbitrEvent`** (парсер `_parse_event_node`): `kind`=`.case-type`=ТИП акта; `title`=`.case-subject`=**ФИО СУДЬИ/стороны** (НЕ описание!); `description`=`.additional-info`+результат=суть. В уведомлении берём `kind`+`description`, НЕ `title` (был баг: показывал судью вместо акта). 🛑 В тексте markdown-ссылки экранируем `[]`→`()` (имена файлов kad бывают «[Подписано] …»). 🛑 Лимит текста MAX — 4000 симв. (режем по границе строки). 🛑 Парсер kad — на **проде**; при деплое кроме `web`/`celery` рестартить **`arbitr-runner` a/b/c/d** вручную (deploy-handler их не трогает).

**Привязка MAX сотрудника — bot + одноразовый код** (у Telegram такого флоу НЕ было, это первый): `Employee.max_chat_id` (CharField unique, миграция `core/0034`; раньше max_chat_id был только у `Client`). Профиль `/profile/`: карточка «Уведомления в MAX» (`_profile_max.html` + view `core:profile_max`) → «Получить код» → `apps/maxchat/linkcode.py:issue()` кладёт 6-симв. код в Redis на 15 мин → сотрудник шлёт код нашему MAX-боту → webhook `handle_max_event` ПЕРЕД созданием `Client` зовёт `linkcode.claim(text)` → `_bind_employee_max()` сохраняет `user_id` в `max_chat_id` + подтверждение. Опц. env `MAX_BOT_LINK` — ссылка на бота в инструкции. Чек в профиле правит `ProfileForm.notify_court_events_max`.
