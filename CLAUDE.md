...
Секретарь сканирует бумаги на офисном МФУ; сканы попадают в лоток «Входящие сканы» (`/scans/`), где привязываются к клиенту. **В проде, боевой приём включён** (`SCAN_AGENT_TOKEN` в `.env.prod`); флаг `can_handle_scans` выдан Юр.отделу, Отделу сбора документов, Бухгалтерии.

- **Канал приёма — локальный агент, НЕ поллинг.** Сервер в облаке не видит офисную сеть, поэтому файлы *пушит* скрипт `tools/scan-agent/` (watchdog следит за SMB-папкой МФУ → `POST /scans/agent/intake/` с Bearer-токеном `SCAN_AGENT_TOKEN`). IMAP/беат-таска не нужны. Эндпоинт `apps/scans/agent.py` (паттерн Bearer как у DevOps-агента) + WS-тост сотрудникам `can_handle_scans` при приёме.
- **Модель `IncomingScan`** (status pending/assigned/discarded, source agent/manual) — холдинг-запись поверх `files.StoredFile`. Файл льётся в S3 prefix `scans/inbox`. «Отклонить» удаляет файл из S3, запись → архив.
- **UI лотка** (`templates/scans/`, `apps/scans/views.py`): лента с inline-превью PDF, поиск по имени/фильтр по источнику/вкладка «Архив», бейдж непривязанных в меню (`pending_count`, поллинг+`scansChanged`). Привязка (модалка): поиск клиента + папка (авто-папка `slug="scans"`) + опц. **минимальная** форма корреспонденции (услуга + контрагент-**typeahead** `counterparty_search` + тип + дата; `Correspondence.file_link` = inline-маршрут). **Пакетная привязка**: чекбоксы → `batch_modal`/`batch_assign` (один клиент/папка). 🛑 hx-include фильтров скоупить к `#scans-inbox` (в DOM несколько `name=q`).
- **Доступ — флаг `Employee.can_handle_scans`** (+ admin/head_dep). Хелпер `apps.core.permissions.can_handle_scans`. Видимость пункта меню — вручную в `context_processors.sidebar_menu` (у `MenuItem` своих флагов нет). Сидинг меню: `python manage.py scans_seed`.
- **Привязка к сотруднику — `Employee.scanner_name`** (= `device` в config.ini агента). По умолчанию сотрудник видит сканы со своим `scanner_name`; выпадашка «сканер» в шапке лотка даёт смотреть чужие + «Все» (это **фильтр-селектор, не изоляция** — юрист может глянуть сканы секретаря). Пустой `scanner_name` → дефолт «Все». Бейдж/тост — по своему сканеру. В проде назначено **по одному сканеру на отдел**: `scanner-jur` (Юр.отдел), `scanner-docs` (Отдел сбора документов), `scanner-buh` (Бухгалтерия) — по 2 сотрудника; на офисных ПК `device` в config.ini должен совпадать. 🛑 В агенте `device` ОБЯЗАТЕЛЕН (ошибка при инициализации, если пуст).
- 🛑 **Превью**: scan-agent шлёт файл как `application/octet-stream` → если сохранить как есть, inline-превью скачивается вместо показа. Лечится в `files.download_stored_file` (при пустом/octet-stream типе угадывает по имени) + intake сразу сохраняет угаданный тип.
- Установка агента — `tools/scan-agent/README.md`. Трей-приложение (`pystray`): индикатор + окно настроек + лог (трей→«Открыть лог», в логе строка `AGENT_BUILD` — версия) + авто-очистка `done_dir`. На сетевых папках — `PollingObserver`. Лаунчеры (Windows): `start_agent.bat` (двойной клик), `start_agent.vbs` (без вспышки окна), `install_autostart.bat` / `remove_autostart.bat` (добавить/убрать из автозагрузки). Все через `pythonw` (без консоли). 🛑 «Сохранить» в настройках перечитывает только config, не код — для новой версии полный перезапуск процесса.
- 🛑 **Токен/адрес — парой**: prod `https://siricrm.ru/scans/agent/intake/` ↔ `SCAN_AGENT_TOKEN` из `.env.prod`; dev `https://crmsiri.ru/...` ↔ токен из `.env.dev`. Рассинхрон url/токена → «CRM отклонила токен» (401). Токен живёт только в `.env.*` (не в git), деплои его не трогают.
