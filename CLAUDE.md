...
Кратко (Этап 1, **на dev и проде с 15.06.2026**): самый крупный раздел — веб-аналог «ПАУ». Карточка дела по услуге БФЛ, полноэкранный своп в `#content-area` из модалки услуги (кнопка «🗂 Карточка процедуры банкротства», право `can_access_procedures`). **Вход также пунктом меню «Юрист БФЛ»** (иконка-Фемида `static/icons/line/femida.svg`, секция «Инструменты», видимость по `can_access_procedures` в `context_processors`, сид меню — миграция `procedure/0008_seed_menu`) → лендинг `/procedure/` (пустая карточка); дело клиента открывается кнопкой **«⚖ Дело БФЛ»** в результатах глобального поиска (`open_client_case`: 1 услуга → карточка, несколько → выбор, нет → подсказка). **Каталог мероприятий правится в Справочниках** (вкладка «Шаблоны мероприятий», `procedure:references_milestones`, гейт `is_references_access`) — не только в админке. **Запросы/ответы в госорганы** (вкладка «Корреспонденция», на проде): модели `RequestType`/`RequestPackage`/`Request` + `ArbitrationManager` (справочник АУ — один PNG подпись+печать). Реестр: исх. № (сквозной по делу, присваивается при создании) + дата создания, госорган (typeahead `LegalEntity`), создание одиночного/пакетом, отметка отправки → авто-срок ответа, контроль просрочки (beat `procedure.mark_overdue_requests` → событийка `request_overdue`). **Документ запроса — 3 способа** (иконки-действия в стиле CRM, серые→цветные на hover, `.req-act`): 📄 сформировать через **AFD** по .docx-шаблону (`request_documents.py` — плейсхолдеры из дела/АУ/анкеты, опц. наложение подписи PNG, авто-PDF; перед генерацией — предпроверка данных, модалка зелёным/красным + «всё равно продолжить»), 📎 подгрузить готовый **pdf/docx** (docx → авто-PDF), ✎ онлайн-правка текста по абзацам. **Ответ на запрос — 3 точки входа** (ставит `response_scan`+статус «ответ»+дата): 📥 в реестре, галочка «ответ на запрос» в загрузке Входящих, и в модуле сканера (`scans` — `client_targets`/`assign`). **Быстрый предпросмотр в модалке** (не новая вкладка, `procPreview`/`procPreviewOffice` в `_tab_correspondence.html`): PDF/сканы/картинки — iframe inline; **doc/docx/xls — Microsoft Office Online Viewer** (как файл-менеджер; нужен сырой публичный pre-signed URL → эндпоинт `procedure:doc_presigned_url`). Типы/шаблоны запросов — команда `load_request_templates` (читает `OLD/Шаблоны запросов/*.docx` → S3 + создаёт `RequestType`, идемпотентно). Справочники: «Типы/Пакеты запросов», «Арбитражные управляющие». **Корреспонденция**: Входящие/Исходящие/**Судебные акты** (вложения из дела kad `apps/arbitr`) — аккордеоны (по умолч. свёрнуты, состояние `<details>` помнится через `_detToggle`) + загрузка сканов (контрагент typeahead с фильтром по типу ЮЛ и галочкой «по региону дела»). Вкладка **Суд**: кнопки «найти/парсить сейчас» + прогресс-бар парсинга kad (`arbitr:case_block_status`). **Двухуровневая модель:** `BankruptcyCase` (1:1 к `crm.Service`) несёт общие стадии (Подготовка→Подача→Принятие/первое заседание) + итог 1-го заседания; дочерние `Procedure` (реструктуризация/реализация — их может быть несколько) со своими стадиями, датами (введение, **публикации ЕФРСБ и Коммерсантъ — две РАЗНЫЕ даты**), ФУ и исходом. Стадии/мероприятия-сроки — **данные** (`ProcedureStage`/`MilestoneTemplate`, DRAFT-сид, правит АУ), не хардкод. `ProcedureMilestone` — снапшот правила, `due_date = базовая дата + offset_days`; beat `procedure.mark_overdue_milestones` метит просрочку + событийка/уведомление. **Таймлайн стадий — в шапке карточки** (фазы: до введения / процедура(ы) / окончание; узлы шириной под название; авто-обновление через `HX-Trigger: procStagesChanged`). Данные должника/супруга правят карточку `Client` (телефоны/адреса CRUD scoped по `who`; супруг — выбор в 2 шага с явным «Сохранить»). Движок — чистые функции в `services.py` (`ensure_case` ленивый, `add_procedure`, `enter_stage`, `recompute_due_dates`, автозакрытие по терминальным исходам). 🛑 `@never_cache` на GET-партиалах; daisyUI timeline/collapse вырезаны (свой CSS/JS); ФУ — на процедуре. **На проде с 15.06.2026** (DRAFT-каталог мероприятий — подтвердить с АУ); запросы/корреспонденция/предпросмотр выкачены 21.06.2026. 🛑 `procedure_seed` и `load_request_templates` НЕ входят в deploy-handler — гонять вручную при выкатке (load_request_templates нужны .docx из `OLD/Шаблоны запросов/` — закоммичены в репо). Память: `procedure-module`, `strict-direction-procedure`.
