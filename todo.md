...
3. 🟡 На fo-y.ru висят ещё два битых внешних скрипта: `sflog.ru` (просроченный
   сертификат) и `js.aacaw.com` — проверить, что это, второй похож на инъекцию.
4. 🟢 Эндпоинт публичный и создаёт Init кому угодно (как и прежняя форма) —
