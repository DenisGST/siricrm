...
Ревизия: **29.08.2026** (предыдущая — 20.08.2026).
...
| # | Симптом | Где | Статус |
|---|---|---|---|
| 1 | **Оплата с сайта fo-y.ru не проходит** — посетитель жмёт «Оплатить» по 5–10 раз, платежа нет | fo-y.ru + [apps/accounting](apps/accounting/) | ✅ закрыто 21.08: CRM на проде + страница сайта переведена на серверный Init |
...
**Выкачено на прод 20.08** (`ea0d6f3`). **Страница fo-y.ru переведена 21.08** — форма шлёт POST
на `/accounting/acquiring/pay/`, виджет и `terminalkey` со страницы убраны. Проверено сквозным
тестом в реальном Chrome и на десктопе, и с эмуляцией телефона: форма → CRM → `pay.tbank.ru`.

🛑 **Гочча SP Page Builder:** контент страницы лежит в БД в **двух** колонках
`nq1gm_sppagebuilder` — `content` (структура, из неё рендерится фронт) и `text` (скомпилированный
вариант). Правка через админку может не сохраниться и остаться незамеченной: страница
продолжает отдавать старое. Проверять по факту — `curl … | grep acquiring/pay`, а не по виду
редактора. Правили обе колонки, бэкап строки — `~/backup-claude/page16-before-*.sql`.

**Осталось:**
1. 🟢 Эндпоинт публичный и создаёт Init кому угодно (как и прежняя форма) —
   при случае добавить троттлинг по IP.
2. 🟢 Через пару дней сверить конверсию: попыток (`AcquiringPrepay`) к оплатам должно стать
   примерно 1:1, как было в июле, вместо нынешних 30:1.
...
## 🟢 Collabora Online — включена на проде, остались мелочи

**Сделано:** онлайн-правка .docx по WOPI ([apps/procedure/wopi.py](apps/procedure/wopi.py)),
контейнер `collabora` в обоих compose, блок для системного nginx —
[nginx/collabora-host.conf.example](nginx/collabora-host.conf.example), пересборка PDF по
кнопке (`pdf_is_stale` + бейдж «PDF устарел»), кнопка правки прямо из строки таблицы.
**Включена на проде 29.08** (контейнер здоров, discovery через nginx отдаёт 200, в логах
`Adding trusted WOPI host: [web]`, WOPI-вызовы проверены).

**Осталось:**
1. 🟡 **Проверить сессию правки в браузере** — единственный участок, который не удалось
   покрыть автотестом: Collabora возвращает `access_token` из POST-формы, синтетический
   WS-клиент это не воспроизводит. Всё до него проверено на обоих серверах.
   Диагностика при сбое: `docker logs siricrm-collabora-1 | grep -i "CheckFileInfo\|unauthorized"`.
2. 🟢 Проверить лимиты бесплатной редакции CODE (порядка 10 документов / 20 подключений
   одновременно) на актуальной версии — для отдела юристов должно хватать, но цифру не сверяли.
3. 🟢 `rebuild` на проде поднимет `collabora` сам и потянет ~1.9 ГБ образа — не ломает
   ничего, но про диск и трафик знать надо.
