...
| 2 | **Компрометация хостинга fo-y.ru**: JS-инъекция `sflog.ru`, а под ней — веб-шеллы с RCE на двух сайтах аккаунта | Sprinthost `gradstatio` | ✅ вычищено 21.08, 🔴 **остались смена паролей и обновление SP Page Builder** — [разбор](docs/foy-injection-2026-08.md) |
...
🛑 **Под инъекцией оказалось хуже:** в 12 каталогах `media/com_sppagebuilder/assets/iconfont/*/fonts/`
на **обоих** сайтах аккаунта (fo-y.ru и ak-siri) лежали веб-шеллы — включая бэкдор с `system()`
от 14.08.2026 и сборщик паролей, который 02.07 выгрузил доступы к базе в JSON прямо на сервер.
Точка входа — загрузка иконочных шрифтов SP Page Builder 5.1.7.

**Сделано 21.08 (по SSH):** сняты инъекция и XSS-класс (правка в БД поверх полного дампа),
51 файл атакующего перенесён в карантин `~/quarantine-claude-20260821-105134/`, чистота
проверена по файловой системе и в реальном Chrome. Сайты работают.

**🔴 Осталось — только вы:**
1. Сменить пароли: хостинг-панель, SSH/FTP, MySQL обеих баз, админки Joomla. Пароль БД
   у атакующего с 02.07 — без смены вернутся без всяких шеллов.
2. Обновить SP Page Builder и Joomla на обоих сайтах — иначе зальют снова.
3. Написать в Sprinthost: в `/tmp` лежит `.htaccess` чужого аккаунта (`webshopnny`) — задет
   не только наш; попросить проверку сервера и логи за 22.06–14.08.
4. Включить журнал действий Joomla (`nq1gm_action_logs` пуст — истории правок нет).

Наши `siricrm.ru`/`crmsiri.ru` — чисто.
