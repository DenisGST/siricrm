...
Fallback добавлен в `apply_files` (зеркалит остальные applier'ы). **Выкачен на прод 29.08.2026** (коммит `ac598f8`); `devops-runner` перезапущен вручную — деплой его не трогает, а именно в нём гоняется apply Files.
